...
func (u *User) SetNameAndAge(n string, a int) {
	if n == "" {
		fmt.Println("Name cannot be empty")
		return
	}
...
func main() {
	...
	u1.SetNameAndAge("vamshi", 24)
	u2.SetNameAndAge("sai", 22)
	u3.SetNameAndAge("saivamshi", 20)

	fmt.Println(u1.GetNameAndAge())
	fmt.Println(u2.GetNameAndAge())
	fmt.Println(u3.GetNameAndAge())
}
